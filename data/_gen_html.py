...
<div class="page active" id="page-1">
  <div class="card">
    ...
    <p class="text-muted" style="margin-bottom:12px">ポケモン名が選択された枠のみ登録されます。</p>
    <div id="party-slots"></div>
    <div class="notice" style="margin-top:2px">未使用の枠は空欄のまま次へ進んでください。</div>
  </div>
  <div class="btn-row"><button class="btn btn-primary" onclick="submitParty()">次へ：相手パーティ入力 →</button></div>
</div>
<div class="page" id="page-2">
  <div class="card">
    ...
    <div class="grid grid-2">
      <div class="field"><label>相手1体目</label><input type="text" id="opp0" placeholder="例: カイリュー" list="poke-datalist"></div>
      <div class="field"><label>相手2体目</label><input type="text" id="opp1" placeholder="例: ドヒドイデ" list="poke-datalist"></div>
      <div class="field"><label>相手3体目</label><input type="text" id="opp2" placeholder="例: テッカグヤ" list="poke-datalist"></div>
      <div class="field"><label>相手4体目</label><input type="text" id="opp3" placeholder="例: ミミッキュ" list="poke-datalist"></div>
      <div class="field"><label>相手5体目</label><input type="text" id="opp4" placeholder="例: ウーラオス" list="poke-datalist"></div>
      <div class="field"><label>相手6体目</label><input type="text" id="opp5" placeholder="例: トゲキッス" list="poke-datalist"></div>
    </div>
    <datalist id="poke-datalist"></datalist>
      ...
      <div class="card">
        ...
        <div class="turn-section">
          ...
          <div class="collapsible" id="ability-section">
            ...
            <div class="field" style="margin-top:5px"><label>発動ポケモン</label><input type="text" id="ab-poke" placeholder="例: カイリュー" list="poke-datalist"></div>
          </div>
        </div>
        <div class="turn-section">
          ...
          <div class="grid grid-2">
            ...
            <div>
              ...
              <div class="collapsible" id="opp-switch-section"><input type="text" id="opp-switch-to" placeholder="交代先のポケモン名" style="margin-top:5px" list="poke-datalist"></div>
            </div>
          </div>
        </div>
        <div class="turn-section">
...
(()=>{const dl=document.getElementById('poke-datalist');POKEMON_DATA.forEach(p=>{const o=document.createElement('option');o.value=p.n;dl.appendChild(o);});})();
...
function goStep(n){document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));document.getElementById('page-'+n).classList.add('active');for(let i=1;i<=4;i++)document.getElementById('step-ind-'+i).className='step'+(i===n?' active':(i<n?' done':''));currentStep=n;}
...
function onPokeSelect(i){const n=document.getElementById(`poke-namejp-${i}`).value;updateAbilitySelect(i,n);updateItemSelect(i,n);updateSlotSummary(i);}
...
function updateItemSelect(i,nameJp){const sel=document.getElementById(`poke-item-${i}`);if(!sel)return;const mg=POKE_MEGA[nameJp];let o='<option value="">なし</option>';if(mg&&mg.length){o+='<optgroup label="メガストーン">';mg.forEach(m=>{o+=`<option value="${m.stone_jp}">${m.stone_jp}（${m.mega_jp}）</option>`;});o+='</optgroup><optgroup label="通常アイテム">';ITEMS_LIST.forEach(it=>{o+=`<option value="${it}">${it}</option>`;});o+='</optgroup>';}else ITEMS_LIST.forEach(it=>{o+=`<option value="${it}">${it}</option>`;});sel.innerHTML=o;}
...
(()=>{const dl=document.createElement('datalist');dl.id='moves-datalist';MOVES_LIST.forEach(m=>{const o=document.createElement('option');o.value=m;dl.appendChild(o);});document.body.appendChild(dl);})();
function addPokeSlot(){if(slotCount>=6)return;const i=slotCount++;const c=document.getElementById('party-slots');const div=document.createElement('div');div.className='poke-slot';div.id=`slot-${i}`;const po='<option value="">選択してください</option>'+POKEMON_DATA.map(p=>`<option value="${p.n}">${p.n}</option>`).join('');div.innerHTML=`<div class="poke-slot-header" onclick="toggleSlot(${i})"><div class="slot-num">${i+1}</div><div class="slot-summary" id="slot-summary-${i}"><span style="color:var(--text3)">クリックして入力</span></div><div class="slot-expand">›</div></div><div class="poke-slot-body"><div class="grid grid-2" style="margin-bottom:8px"><div class="field"><label>ポケモン名</label><select id="poke-namejp-${i}" onchange="onPokeSelect(${i})">${po}</select></div><div class="field"><label>性別</label><select id="poke-gender-${i}"><option value="-">なし（-）</option><option value="♂">♂</option><option value="♀">♀</option></select></div></div><div class="grid grid-2" style="margin-bottom:8px"><div class="field"><label>持ち物</label><select id="poke-item-${i}"><option value="">なし</option></select></div><div class="field"><label>特性</label><select id="poke-ability-${i}"><option value="">選択してください</option></select></div></div><div style="margin-bottom:8px"><div class="field"><label>性格</label>${buildNatureSelect('poke-nature-'+i)}</div></div><div style="margin-bottom:8px"><label style="margin-bottom:5px">努力値（H/A/B/C/D/S）</label><div class="ev-row">${['H','A','B','C','D','S'].map(s=>`<div><div class="ev-label">${s}</div><input type="number" id="poke-ev-${s}-${i}" min="0" max="252" value="0"></div>`).join('')}</div></div><div><label style="margin-bottom:5px">技（最大4つ）<span style="font-size:11px;color:var(--text3);margin-left:6px">※リストにない技は赤枠</span></label><div class="moves-row">${[1,2,3,4].map(m=>`<div class="field"><label>技${m}</label><input type="text" id="poke-move${m}-${i}" placeholder="例: じしん" list="moves-datalist" oninput="validateMove(this)"></div>`).join('')}</div></div></div>`;c.appendChild(div);toggleSlot(i);updateItemSelect(i,'');}
...
async function submitParty(){const party=[];for(let i=0;i<slotCount;i++){const n=document.getElementById(`poke-namejp-${i}`)?.value||'';if(!n)continue;const p=POKE_MAP[n];party.push({name_jp:n,name_en:p?p.e:n,item:document.getElementById(`poke-item-${i}`)?.value||null,ability:document.getElementById(`poke-ability-${i}`)?.value||null,gender:document.getElementById(`poke-gender-${i}`)?.value||'-',nature:document.getElementById(`poke-nature-${i}`)?.value||'まじめ',evs:{H:+document.getElementById(`poke-ev-H-${i}`)?.value||0,A:+document.getElementById(`poke-ev-A-${i}`)?.value||0,B:+document.getElementById(`poke-ev-B-${i}`)?.value||0,C:+document.getElementById(`poke-ev-C-${i}`)?.value||0,D:+document.getElementById(`poke-ev-D-${i}`)?.value||0,S:+document.getElementById(`poke-ev-S-${i}`)?.value||0},moves:[1,2,3,4].map(m=>document.getElementById(`poke-move${m}-${i}`)?.value.trim()||'').filter(Boolean)});}if(party.length===0){showError('ポケモンを少なくとも1体選択してください');return;}try{await api('/setup/party','POST',{my_party:party});myPartyData=party;goStep(2);}catch(e){showError(e.message);}}
...
function clearTurnForm(){['has-ability','has-my-switch','has-opp-switch'].forEach(id=>document.getElementById(id).checked=false);['ability-section','my-switch-section','opp-switch-section'].forEach(id=>document.getElementById(id).classList.remove('open'));['ab-name','ab-poke','opp-switch-to','opp-move','my-item-used','opp-item-used'].forEach(id=>document.getElementById(id).value='');document.getElementById('opp-move-my-hp').value='';document.getElementById('my-hp-input').value='';['my-status-change','opp-status-change'].forEach(id=>document.getElementById(id).value='');}
...
async function endBattle(){if(!confirm('バトルを終了してリセットしますか？'))return;try{await api('/battle/reset','POST');}catch(e){}turnNum=1;myPartyData=[];oppPartyNames=[];selectionRec=null;slotCount=0;document.getElementById('party-slots').innerHTML='';document.getElementById('selection-result-area').style.display='none';document.getElementById('goto-step3-btn').style.display='none';document.getElementById('action-log').innerHTML='';document.getElementById('rec-area').innerHTML='<div class="empty-state"><div style="font-size:28px;margin-bottom:8px">⚔</div><div>ターン情報を入力して<br>「推薦を取得」を押してください</div></div>';document.getElementById('opp-estimate-area').innerHTML='<div class="empty-state" style="padding:16px">まだ情報がありません</div>';[0,1,2,3,4,5].forEach(i=>{const el=document.getElementById('opp'+i);if(el)el.value='';});goStep(1);for(let i=0;i<6;i++)addPokeSlot();}
window.onload=()=>{for(let i=0;i<6;i++)addPokeSlot();};
</script>
</body>
</html>"""
